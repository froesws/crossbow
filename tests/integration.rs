...
fn load_test_data() -> DataFrame {
    ...
    let columns: Vec<Series> = schema
        ...
        .iter()
        .enumerate()
        .map(|(i, field)| Series::new(field.name(), batch.column(i).clone()))
        .collect();
...
fn test_data_loading() {
    ...
    assert_eq!(
        df.get_column_names(),
        vec!["employee", "salary", "age", "position", "yield"]
    );
...
fn test_dataframe_sort_ascending() {
    let df = load_test_data();
    let sorted = df.sort_by_asc("age").unwrap();
...
fn test_dataframe_sort_descending() {
    let df = load_test_data();
    let sorted = df.sort_by_desc("yield").unwrap();
...
fn test_dataframe_sort_by_string() {
    let df = load_test_data();
    let sorted = df.sort_by_asc("position").unwrap();
    assert_eq!(sorted.shape(), df.shape());
...
fn test_sort_multiple_columns() {
    let df = load_test_data();
    let by_age = df.sort_by_asc("age").unwrap();
    let by_salary = df.sort_by_desc("salary").unwrap();
...
fn test_shuffle_preserves_shape() {
    ...
    assert_eq!(shuffled.get_column_names(), df.get_column_names());
}
...
fn test_shuffle_preserves_row_cohesion() {
    ...
    for i in 0..shuffled.shape().0 {
        ...
        assert!(
            ...
            "Shuffled row not found in original"
        );
    }
}

#[test]
fn test_shuffle_then_sort_restores() {
    let df = load_test_data();
    let shuffled = df.shuffle().unwrap();
    let sorted_after = shuffled.sort_by_asc("age").unwrap();
    let sorted_original = df.sort_by_asc("age").unwrap();

    assert_eq!(sorted_after.shape(), sorted_original.shape());
    let after_ages = sorted_after.select("age").unwrap();
    let orig_ages = sorted_original.select("age").unwrap();
    for i in 0..after_ages.len() {
        assert_eq!(
            after_ages.value_at(i).unwrap(),
            orig_ages.value_at(i).unwrap()
        );
    }
}
...
#[test]
fn test_date32_join_integration() {
    let left = DataFrame::new(vec![
        Series::from("id", vec![1i32, 2, 3]),
        Series::from_date32("d", vec![19700, 19701, 19702]),
    ])
    .unwrap();

    let right = DataFrame::new(vec![
        Series::from("id", vec![1i32, 2, 4]),
        Series::from("val", vec![10i32, 20, 40]),
    ])
    .unwrap();
...
#[test]
fn test_date32_groupby_min_max_integration() {
    let cat = Series::from("cat", vec!["a", "b", "a"]);
    let dates = Series::from_date32("d", vec![19701, 19700, 19702]);
    let df = DataFrame::new(vec![cat, dates]).unwrap();
    let grouped = df.group_by("cat").unwrap();

    let min_result = grouped.min("d").unwrap();
    assert_eq!(min_result.shape().0, 2);
...
#[test]
fn test_date_integration_sort_and_filter() {
    let labels = Series::from("label", vec!["x", "y", "z", "w"]);
    let dates = Series::from_date32("d", vec![19702, 19700, 19701, 19703]);
    let df = DataFrame::new(vec![labels, dates]).unwrap();

    let sorted = df.sort_by_asc("d").unwrap();
    let mask = Series::from("mask", vec![true, false, true, false]);
    let filtered = sorted.filter_by_mask(&mask).unwrap();
